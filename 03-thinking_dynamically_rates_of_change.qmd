...
heart_data <- data.frame(Time = rep(x, times = 3), 
                         ...
                         Type = rep(x = c("Observed HR", "True HR", "Derivative (or Impulse)"), each = 2000))
...
ggplot(heart_data, aes(Time, HR, col = Type)) +
    ...
    geom_hline(data = data.frame(yintercept = 0, Type = "Derivative (or Impulse)", HR = 0, Time = 0), aes(yintercept = yintercept),
               linetype = 2) +
    geom_line() +
